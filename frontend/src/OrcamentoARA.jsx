...
const PLANO_CONTAS_AGRICOLA = {
  pessoal: [
    ...
    { codigo: '71101009', nome: "VALE ELETRONICO (VEM)", origem: 'Custo' },
    { codigo: '71101010', nome: "CESTAS BASICAS", origem: 'Custo' },
    { codigo: '71101011', nome: "FARDAMENTOS", origem: 'Custo', individual: true },
    { codigo: '71101012', nome: "ASSISTENCIA MEDICA E SOCIAL", origem: 'Custo' },
    { codigo: '71101013', nome: "DESPESAS COM TREINAMENTO DE PESSOAL", origem: 'Custo', individual: true },
    { codigo: '71101014', nome: "PENSAO ALIMENTICIA", origem: 'Custo' },
    ...
    { codigo: '34101008', nome: "VALE ELETRONICO (VEM)", origem: 'Despesa' },
    { codigo: '34101009', nome: "CESTAS BASICAS", origem: 'Despesa' },
    { codigo: '34101010', nome: "FARDAMENTOS", origem: 'Despesa', individual: true },
    { codigo: '34101011', nome: "ASSISTENCIA MEDICA E SOCIAL", origem: 'Despesa' },
    { codigo: '34101012', nome: "TREINAMENTO DE PESSOAL", origem: 'Despesa', individual: true },
    { codigo: '34101014', nome: "HORAS EXTRAS", origem: 'Despesa' },
    { codigo: '34101015', nome: "ALIMENTACAO", origem: 'Despesa', individual: true },
    ...
    { codigo: '34201015', nome: "ALIMENTACAO", origem: 'Despesa', individual: true },
    { codigo: '34201019', nome: "OUTRAS DESP COM FUNCIONARIOS", origem: 'Despesa' },
    { codigo: '34201022', nome: "INSALUBRIDADE", origem: 'Despesa' },
    { codigo: '34202009', nome: "DESPESA  ALIMENTACAO", origem: 'Despesa', individual: true },
    { codigo: '34202013', nome: "DESPESAS COM FESTAS E COMEMORACOES", origem: 'Despesa', individual: true },
    { codigo: '34202021', nome: "REFEICOES", origem: 'Despesa', individual: true },
...
// Resorts_Contas_x_Pacote (Matriz_Governanca_OBZ_2027_4.xlsx): 260 "contas"
// classificadas, mas 158 delas eram na verdade contas da Têxtil/Agrícola
// (mesmo código 71xxx/72xxx/34xxx usado nos planos dessas duas unidades) —
// contaminação na planilha-fonte, identificada em 2026-09-07 ao notar
// MATERIA PRIMA FIO/MALHARIA/BENEFICIAMENTO (contas têxteis) aparecendo no
// pacote Produção dos Resorts. Corrigido cruzando com o "Plano de Contas
// Resorts.xlsb" oficial (fornecido pelo usuário na mesma data): as contas
// genuínas dos Resorts usam sempre código 41xxx/51xxx. As 158 contaminadas
// foram removidas (só depois de migrar os valores já preenchidos pelos
// gestores de Samoa Beach/Villa para a conta genuína equivalente — ver
...
const PLANO_CONTAS_RESORTS = {
  ...
  comercial: [
    ...
    { codigo: '510491020', nome: "COMISSOES DE AGENTE SCP", origem: 'Custo' },
    { codigo: '510491030', nome: "OUTRAS DESPESAS OPERACIONAIS SCP", origem: 'Custo' },
  ],
  manutencao: [
...
const TIPOS_PREMISSA = [
  ...
  { id: 'rateio_aeb', nome: 'Base × % Receita A&B' },
...
function valorSublinhaMes(sublinha, m, receitaBrutaMes, receitaLiquidaMes, ipcaAnualPct, volumeTotalKgMes, receitaHospedagemMes, receitaAebMes) {
...
function valorLinhaMes(contaRaw, m, receitaBrutaMes, receitaLiquidaMes, ipcaAnualPct, volumeTotalKgMes, receitaHospedagemMes, receitaAebMes) {
  if (!contaRaw) return 0;
  const conta = normalizarConta(contaRaw);
  return conta.sublinhas.reduce((acc, sub) => acc + valorSublinhaMes(sub, m, receitaBrutaMes, receitaLiquidaMes, ipcaAnualPct, volumeTotalKgMes, receitaHospedagemMes, receitaAebMes), 0);
}
function valorLinhaAnual(contaRaw, receitaBrutaMes, receitaLiquidaMes, ipcaAnualPct, volumeTotalKgMes, receitaHospedagemMes, receitaAebMes) {
...
function valorSublinhaMesCaixa(sublinha, m, receitaBrutaMes, receitaLiquidaMes, ipcaAnualPct, volumeTotalKgMes, receitaHospedagemMes, receitaAebMes) {
  if (!sublinha) return 0;
...
function valorLinhaMesCaixa(contaRaw, m, receitaBrutaMes, receitaLiquidaMes, ipcaAnualPct, volumeTotalKgMes, receitaHospedagemMes, receitaAebMes) {
  if (!contaRaw) return 0;
  const conta = normalizarConta(contaRaw);
  return conta.sublinhas.reduce((acc, sub) => acc + valorSublinhaMesCaixa(sub, m, receitaBrutaMes, receitaLiquidaMes, ipcaAnualPct, volumeTotalKgMes, receitaHospedagemMes, receitaAebMes), 0);
}
...
function computeFolhaPessoalMes(funcionariosCC, premissas, mIdx) {
  ...
  // Decisão de 2026-09-23: contratação de 2027 (Novo Headcount, único uso
  // desta função) não é elegível à meritocracia no ano. A meritocracia vale
  // só para o Headcount Existente, a partir de meritocraciaMes.
...
function runAuditoria(data, dre, ref, unidadeId, ipcaAnualPct) {
  ...
  // o campo como obrigatório para todas as unidades... deixe o campo, mas
...
function LinhaSublinha({ sublinha, onUpdate, unidadeId, ipcaAnualPct, volumeTotalKgMes, receitaBrutaMes, receitaLiquidaMes, cambios, receitaHospedagemMes, receitaAebMes }) {
  const valoresMensaisCalc = MESES.map((_, m) => valorSublinhaMes(sublinha, m, receitaBrutaMes, receitaLiquidaMes, ipcaAnualPct, volumeTotalKgMes, receitaHospedagemMes, receitaAebMes));
  ...
  // e "Base × % Receita A&B" só Resorts — ver UNIDADES_COM_CUSTO_POR_KG /
  // UNIDADES_COM_RATEIO_HOSPEDAGEM.
  const opcoesPremissa = TIPOS_PREMISSA.filter(t =>
    ...
    (t.id !== 'rateio_aeb' || UNIDADES_COM_RATEIO_HOSPEDAGEM.includes(unidadeId))
  );
...
// sem migração.
function LinhaConta({ conta, linha, aberta, onToggle, onUpdateClassificacao, onUpdateSublinha, onAddSublinha, onRemoveSublinha, total, receitaBrutaMes, receitaLiquidaMes, ocultarClassificacao, ocultarAddSublinha, unidadeId, ipcaAnualPct, volumeTotalKgMes, cambios, linhasCalculadas, observacao, receitaHospedagemMes, receitaAebMes }) {
          ...
          {contaNorm.sublinhas.map((sub, i) => (
            <div key={sub.id} style={{ border: multiplas ? `1px solid ${COR.borda}` : 'none', borderRadius: multiplas ? 6 : 0, padding: multiplas ? 8 : 0, marginBottom: multiplas ? 10 : 0, background: multiplas ? COR.claro : 'transparent' }}>
              ...
              <LinhaSublinha
                ...
                cambios={cambios} receitaHospedagemMes={receitaHospedagemMes} receitaAebMes={receitaAebMes}
              />
            </div>
          ))}
...
function AbaCustos({ refUnidade, unidadeId, usuario, linhas, updateConta, updateSublinha, addSublinha, removeSublinha, dre, ipcaAnualPct, detalhes, addDetalhe, updateDetalhe, removeDetalhe, funcionarios, addFuncionario, updateFuncionario, removeFuncionario, premissasPessoal, updatePremissaPessoal, viagens, atualizar, premissasMacro, cambios }) {
  ...
  // Antes a linha somava mais um % por cima, contando o dissídio duas vezes.
  const _funcsNovoCc = (funcionarios || []).filter(f => f.ccCodigo === ccSel && f.origem === 'novo');
  const folhaNovoSemDissidio = computeFolhaPessoalAnual(_funcsNovoCc, { ..._ppC, dissidioMes: '', dissidioMes2: '' });
  const folhaNovoSoDissidio1 = computeFolhaPessoalAnual(_funcsNovoCc, { ..._ppC, dissidioMes2: '' });
  const _acrescimo = (com, sem) => MESES.map((_, m) => com.mensal[m].total - sem.mensal[m].total);
  const dissidioNovoHcRowCorp = unidadeId === 'corporativo' && _dissidioMesIdxC >= 0 && parseNum(_ppC.dissidioPct)
    ? _acrescimo(folhaAtual, folhaNovoSemDissidio)
    : null;
  const dissidioNovoHcRow1Textil = _hcExisteContasTextil.length > 0 && _dissidioMesIdxC >= 0 && parseNum(_ppC.dissidioPct)
    ? _acrescimo(folhaNovoSoDissidio1, folhaNovoSemDissidio)
    : null;
  const dissidioNovoHcRow2Textil = _hcExisteContasTextil.length > 0 && _dissidioMesIdx2T >= 0 && parseNum(_ppC.dissidioPct2)
    ? _acrescimo(folhaAtual, folhaNovoSoDissidio1)
    : null;
                            ...
                            <div style={{ padding: 10 }}>
                              ...
                              <div style={{ marginTop: 10 }}>
                                <TabelaMensal
                                  linhas={[]} onChangeCelula={() => {}}
                                  linhasCalculadas={[
                                    { key: 'folhaNovo', label: 'Folha — Novo Headcount', valoresMensal: folhaNovoSemDissidio.mensal.map(m => m.total), totalValor: folhaNovoSemDissidio.totalAnual, cor: COR.texto },
                      ...
                      <>
                        ...
                        <div style={{ border: `1px solid ${COR.borda}`, borderRadius: 8, marginBottom: 8, overflow: 'hidden' }}>
                          ...
                          >
                            ...
                            <span style={{ fontSize: 12, fontWeight: 700, color: COR.azul }}>{formatBRL(folhaAtual.totalAnual + _somaRow(encargosNovoHcRowTextil))}</span>
                          </button>
                          {novoHcAberto && (
                            <div style={{ padding: 10 }}>
                              <QuadroPessoal
                              ...
                              {(dissidioNovoHcRow1Textil || dissidioNovoHcRow2Textil || encargosNovoHcRowTextil) && (
                                <div style={{ marginTop: 10 }}>
                                  <TabelaMensal
                                    linhas={[]} onChangeCelula={() => {}}
                                    linhasCalculadas={[
                                      { key: 'folhaNovoTextil', label: 'Folha — Novo Headcount', valoresMensal: folhaNovoSemDissidio.mensal.map(m => m.total), totalValor: folhaNovoSemDissidio.totalAnual, cor: COR.texto },
