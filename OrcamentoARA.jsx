...
export default function OrcamentoARA() {
  const [role, setRole] = useState('gerente');
  const [unidadeAtual, setUnidadeAtual] = useState(UNIDADES[0].id);
  ...
  const carregarUnidade = useCallback(async (idUnidade) => {
    ...
    setErro(null);
    try {
      const r = await window.storage.get(`ara-orc:unidade:${idUnidade}`, true);
      setDados(r ? JSON.parse(r.value) : emptyFormData());
    } catch (e) {
      setDados(emptyFormData());
    }
    try {
      const rv = await window.storage.get(`ara-orc:versoes:${idUnidade}`, true);
      setVersoes(rv ? JSON.parse(rv.value) : []);
    } catch (e) {
      setVersoes([]);
    }
    setCarregando(false);
  ...
  const carregarFPA = useCallback(async () => {
    ...
    for (const u of UNIDADES) {
      try {
        const r = await window.storage.get(`ara-orc:unidade:${u.id}`, true);
        mapa[u.id] = r ? JSON.parse(r.value) : emptyFormData();
      } catch (e) {
    ...
    try {
      const rb = await window.storage.get('ara-orc:backlog', true);
      setBacklog(rb ? JSON.parse(rb.value) : []);
    } catch (e) {
      setBacklog([]);
    }
    try {
      const re = await window.storage.get('ara-orc:etapas', true);
  ...
  useEffect(() => {
    (async () => {
      try {
        const r = await window.storage.get('ara-orc:premissas-macro', true);
  ...
  async function updatePremissaMacroGlobal(id, valor) {
    ...
    try {
      await window.storage.set('ara-orc:premissas-macro', JSON.stringify(novas), true);
    } catch (e) {
  ...
  async function buscarBoletimFocus() {
    ...
    try {
      ...
      try {
        await window.storage.set('ara-orc:premissas-macro', JSON.stringify(atualizadas), true);
      } catch (e) {
        // silencioso
      }
    } catch (e) {
    ...
    const t = setTimeout(async () => {
      try {
        ...
        await window.storage.set(
          `ara-orc:unidade:${unidadeAtual}`,
          JSON.stringify({ ...dados, meta: { ...dados.meta, status, atualizadoEm: new Date().toISOString() } }),
          true
        );
      } catch (e) {
  ...
  async function enviarVersao() {
    ...
    const snapshot = { ...dados, meta: { status: 'enviado', atualizadoEm: timestamp, autor: autorNome.trim() } };
    try {
      let versoesAtuais = [];
      try {
        const rv = await window.storage.get(`ara-orc:versoes:${unidadeAtual}`, true);
        versoesAtuais = rv ? JSON.parse(rv.value) : [];
      } catch (e) { versoesAtuais = []; }
      const d = computeDRE(dados);
      const novaVersao = { id: uid(), timestamp, autor: autorNome.trim(), comentario: comentarioEnvio.trim(), totais: d };
      const novasVersoes = [novaVersao, ...versoesAtuais].slice(0, 20);
      await window.storage.set(`ara-orc:versoes:${unidadeAtual}`, JSON.stringify(novasVersoes), true);
      await window.storage.set(`ara-orc:unidade:${unidadeAtual}`, JSON.stringify(snapshot), true);

      let backlogAtual = [];
      try {
        const rb = await window.storage.get('ara-orc:backlog', true);
        backlogAtual = rb ? JSON.parse(rb.value) : [];
      } catch (e) { backlogAtual = []; }
      const entrada = { id: uid(), unidadeId: unidadeAtual, timestamp, autor: autorNome.trim(), comentario: comentarioEnvio.trim(), totalGeral: d.totalGeral };
      const novoBacklog = [entrada, ...backlogAtual].slice(0, 200);
      await window.storage.set('ara-orc:backlog', JSON.stringify(novoBacklog), true);
  ...
  async function abrirDrill(idUnidade) {
    ...
    try {
      const rv = await window.storage.get(`ara-orc:versoes:${idUnidade}`, true);
      setVersoesDrill(rv ? JSON.parse(rv.value) : []);
    } catch (e) {
      setVersoesDrill([]);
    }
  }
  ...
  async function atualizarEtapa(id, campo, valor) {
    ...
    try {
      await window.storage.set('ara-orc:etapas', JSON.stringify(novasEtapas), true);
    } catch (e) {
      // silencioso — próxima gravação tenta novamente
    }
  }
